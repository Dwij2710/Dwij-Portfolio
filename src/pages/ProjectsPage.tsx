...
import { ExternalLink, Cpu, ArrowUpRight, ArrowRight } from 'lucide-react'
      ...
      <div className="max-w-content">
        ...
        <div className="mb-10">
          ...
          <p className="font-mono text-sm text-data mt-2">
            Quantitative Finance Models, Reinforcement Learning, and Enterprise Inference Engines
          </p>
        </div>
        ...
        <div className="space-y-12">
          {projects.map((p) => (
            <article
              key={p.id}
              className="group border border-hairline bg-panel p-6 sm:p-8 rounded-sm hover:border-faint/60 transition-all shadow-sm"
            >
              {/* Header */}
              <div className="flex flex-col sm:flex-row sm:items-baseline sm:justify-between gap-2 border-b border-hairline pb-4">
                ...
                <p className="font-mono text-xs text-data">{p.tagline}</p>
              </div>
              ...
              <div className="mt-6 flex flex-wrap items-center gap-3 border-t border-hairline pt-4">
                <button
                  onClick={() => setActiveProject(p)}
                  className="px-3.5 py-2 bg-panel2 border border-hairline text-ink text-xs font-mono rounded-sm hover:border-signal hover:text-signal transition-colors flex items-center gap-1.5"
                >
                ...
                {p.githubUrl && (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="px-3.5 py-2 border border-hairline text-muted hover:text-ink text-xs font-mono rounded-sm hover:border-faint transition-colors flex items-center gap-1.5"
                  >
                ...
                {p.demoUrl && (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="px-3.5 py-2 border border-hairline text-muted hover:text-data text-xs font-mono rounded-sm hover:border-data/60 transition-colors flex items-center gap-1.5"
                  >
        ...
        <div className="mt-14 flex flex-wrap items-center justify-between gap-4 border-t border-hairline pt-6">
          ...
          >
            <span>Next: Contact & Connect</span>
